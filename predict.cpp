...
void CTNode::updateLogProbability(void) {
	...
	if (m_child[0] == NULL) {
		if (m_child[1] == NULL) {
			...
			prob_weighted = pow(2, m_log_prob_est - 1)
					+ pow(2, m_child[1]->m_log_prob_weighted - 1);
		}
	} else {
		if (m_child[1] == NULL) {
			prob_weighted = pow(2, m_log_prob_est - 1)
					+ pow(2, m_child[0]->m_log_prob_weighted - 1);
		} else {
			prob_weighted = pow(2, m_log_prob_est - 1)
					+ pow(2,
...
void ContextTree::walkAndGeneratePath(int bit_fix,
	...
	int path_size =
			((m_history.size() + bit_fix + m_update_partial_count) < m_depth) ?
					(m_history.size() + bit_fix + m_update_partial_count) :
					m_depth;
	...
	while (traverse_depth < (path_size)) {
		...
		if (m_history.size() == 0 && m_update_partial_count == 0) {
			...
			traverse_depth++;
			break;
		} else if (m_update_partial_count > 0
				&& traverse_depth < m_update_partial_count) {
			cur_history_sym = m_update_partial_list[m_update_partial_count
					- traverse_depth - 1];
