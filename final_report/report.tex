...
Cheesemaze & $11$ & $96$ & $10$ & $0.5$ & $8$ & $0.999$ & $0.99977$ & $1.4$ \tabularnewline
           & $11$ & $96$ & $20$ & $1.6$ & $6$ & $0.999$ & $0.99977$ & $1.4$ \tabularnewline
...
\hline
\end{tabular}
\caption{MC-AIXI-CTW Experiments. Here, $b$ is the total bit length of an $(o,r,a)$ tuple, $D$ is the depth of the context tree, $n_\text{cycles}$ is the maximum number of training cycles (1000's) allowed in the experiment, $t_\text{max}$ is the search time allowed per cycle, $m$ is the search horizon, $\epsilon_0$ is the initial exploration rate, $\gamma$ is the exploration decay rate, and $w_\text{UCB}$ is the UCB exploration bias parameter.}
\end{table}

\subsection{Cheesemaze}
In this cheesemaze run, we chose a very short search timeout of 0.5 seconds, and a moderately long horizon (8). We tried longer timeouts, but this did not improve convergence rates. Using a smaller search horizon of 4 yielded low performance; this is probably due to the fact that in certain maze positions, the cheese was beyond the search horizon of the agent. \\

Within 6000 training cycles, the agent's average reward per cycle is approaching the optimal, and by $10000$ cycles, is exceeding $1.5$. Though cheese maze is the least challenging of our environments, our agent's strong performance and fast convergence suggests that our implementation is correct and reasonably efficient. 
\begin{table}[H]
\centering
...
\subsection{Extended Tiger}
Extended Tiger is a more difficult game to learn than Cheese Maze, since the configuration of the trap/goal changes from game to game, and the reliability of some of the observations is stochastic.  Collecting good results in Extended Tiger is difficult, given that the ideal time for search per cycle is 12 seconds (according to Veness), making good results time consuming to obtain. The game is also very unforgiving, with a very large penalty for opening the tiger door. Considering how bad the penalties are, our agent is reasonably close to optimal, given that extremely negative rewards skew the running average.
\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|l|l|l|l|l|}
\hline 
...
\subsection{Tic\-Tac\-Toe}
Tic-Tac-Toe is a difficult game, because the percept space is very large, and towards the end of each episode, the probability of naively performing an illegal action is very high. For these reasons, it takes a long time to learn how to play Tic-Tac-Toe (Veness's agent took 3 weeks of training to achieve optimality). Given more time, we should expect to see better results on Tic-Tac-Toe, knowing that we have a rising trend and decreasing variance in our average reward per cycle.
\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|l|l|l|l|l|}
\hline 
...
\subsection{Biased Rock-Paper-Scissors}
In biased rock-paper-scissors, we achieve optimal performance after the first $1000$ training cycles. This is unsurprising, given that the agent's optimal strategy is not that far from random, and the agent merely has to learn a simple correlation in its opponent's actions (after all, it is just learning a 1st order Markov process). Both the percept space and action space are small, which means that a context tree of modest depth can model many cycles. Monte Carlo tree search also benefits from this small branching factor, allowing the agent to plan ahead more effectively. Experiments with larger tree depths and longer timeouts did not converge faster as a result; the parameters that we chose were sufficient to be optimal very quickly.
\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|l|l|l|l|l|}
\hline 
...
\subsection{Pacman}
Pacman is clearly a very difficult game. Even if the true environment were known, planning would still be a huge problem as there are $10^60$ unique underlying states. Veness et al. trained for $250,000$ cycles, which is clearly not feasible in our circumstances. Despite these shortcomings, our agent's performance steadily improves, demonstrating that the agent learns to not run into walls and ghosts. 
...
Bits & CT-depth & Cycles & Timeout & Horizon & Exp. Rate & Exp.-Decay & UCB-weight \tabularnewline
\hline 
$26$ & $256$ & $10$ & $2$ & $8$ & $0.9999$ & $0.99977$ & $1.4$ \tabularnewline
\hline
\end{tabular}
\caption{Environment Setup}
\end{table}
...
We ran our cross-domain simulations on Cheesemaze and Biased Rock-Paper-Scissors, since these are simple environments on which we have already achieved optimal or close-to-optimal performance. Moving to RPS, the agent performed extremely well, achieving optimality within the first training phase. This is impressive, given that the domains are quite different; the percept space in RPS is quite smaller, meaning that the patterns learned by the context tree will be quite different. The robustness of our convergence in RPS is perhaps a testament to the ease of the game. However, once we return to the Cheese Maze domain, learning seemed to be hampered, and convergence to optimality was slower than it would otherwise have been. This is perhaps due to the fact that, having collected many statistics in the simple RPS domain has skewed the Cheese Maze agent towards simpler models that don't appropriately represent the maze domain. From our experiment, it is not clear whether models learnt in one domain were beneficial when transferred to other domains.
